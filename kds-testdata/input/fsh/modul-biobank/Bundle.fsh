// ============================================================================
// Biobank Bundle
// ============================================================================

Instance: mii-exa-test-data-bundle-biobank-1
InstanceOf: Bundle
Usage: #example
Description: "Bundle: Biobank Testdaten"
* insert TestDataLabel
* meta.source = "https://www.charite.de/fhir/kds-testdata"
* type = #transaction
* timestamp = "2025-06-18T13:51:00+02:00"
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-1, Patient)
* insert AddBundleEntry(mii-exa-test-data-biobank-encounter-1, Encounter)
// Organization
* insert AddBundleEntry(mii-exa-test-data-organization-biobank-charite, Organization)
// Substance
* insert AddBundleEntry(mii-exa-test-data-patient-1-substance-1, Substance)
// Specimens Patient-1
* insert AddBundleEntry(mii-exa-test-data-patient-1-specimen-1, Specimen)
// Specimens Patient-3
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-3, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-3-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-3-specimen-2, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-3-specimen-3, Specimen)
// Specimens Patient-4
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-4, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-4-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-4-specimen-2, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-4-specimen-3, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-4-specimen-4, Specimen)
// Specimens Patient-5
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-5, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-5-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-5-specimen-2, Specimen)
// Specimens Patient-6
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-6, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-6-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-6-specimen-2, Specimen)
// Specimens Patient-7
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-7, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-7-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-7-specimen-2, Specimen)
// Specimens Patient-8
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-8, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-8-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-8-specimen-2, Specimen)
// Specimens Patient-9
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-9, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-9-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-9-specimen-2, Specimen)
// Specimens Patient-10
* insert AddBundleEntry(mii-exa-test-data-biobank-patient-2, Patient)
* insert AddBundleEntry(mii-exa-test-data-patient-10-specimen-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-patient-10-specimen-2, Specimen)
// Zelllinie/Organoid + SpecimenCore (Profilluecken-Abdeckung, Paket-Beispiele adaptiert)
* insert AddBundleEntry(mii-exa-test-data-biobank-organoid-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-biobank-specimen-dna-1, Specimen)
* insert AddBundleEntry(mii-exa-test-data-biobank-kulturprotokoll-1, DocumentReference)
* insert AddBundleEntry(mii-exa-test-data-biobank-crispr-protokoll-1, DocumentReference)
// Observations zu Bioproben (Profilluecken-Abdeckung, Paket-Beispiele adaptiert)
* insert AddBundleEntry(mii-exa-test-data-biobank-dna-konzentration-1, Observation)
* insert AddBundleEntry(mii-exa-test-data-biobank-karyotyp-1, Observation)
* insert AddBundleEntry(mii-exa-test-data-biobank-morphologie-1, Observation)
* insert AddBundleEntry(mii-exa-test-data-biobank-proliferation-1, Observation)
* insert AddBundleEntry(mii-exa-test-data-biobank-wachstumstyp-1, Observation)
* insert AddBundleEntry(mii-exa-test-data-biobank-qualitaetspruefung-1, Observation)
// Infrastructure
* insert AddBundleEntry(mii-exa-test-data-organization-charite, Organization)
* insert AddBundleEntry(mii-exa-test-data-biobank-diagnose-1, Condition)
